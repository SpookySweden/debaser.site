...
const CHIP =
  'cursor-pointer rounded-none border border-ink bg-paper px-1 py-[1px] font-mono text-[11px] text-ink hover:bg-sun-pale';
...
export default function AsciiEmoticonPicker({ onPick, disabled = false }: AsciiEmoticonPickerProps) {
  ...
  return (
    <div className="relative">
      ...
      {open ? (
        <div className="absolute bottom-full left-0 z-20 mb-1 w-[22rem] rounded-none border-2 border-t-white border-l-white border-r-black border-b-black bg-sun-pale p-2 shadow-none">
          ...
          {ASCII_EMOTICONS.map((group) => (
            <div key={group.id} className="mt-2">
              <p className="text-[10px] font-bold text-ink">{group.label}</p>
          ...
          <p className="mt-2 border-t border-ink pt-1 text-[10px] text-ink">
