...
import { profileComments, validateTagLabel, visibleGivenTags, visibleProfileComments } from '../lib/profile/visibility';
    ...
    <div className="space-y-3">
      ...
      <section className="rounded-none border-2 border-t-white border-l-white border-r-gray-800 border-b-gray-800 bg-[#c0c0c0]">
        ...
        <div className="space-y-2 p-3">
              ...
              {tags.map((tag) => (
                <li key={tag.id} className="flex items-center gap-1">
                  ...
                  <span className="text-[10px] font-bold text-black">
                    {owner && tag.hidden ? '[ HIDDEN ]' : `FROM ${authorLabel(tag.givenBy)}`}
                  </span>
                </li>
              ))}
            </ul>
          ...
          {owner || !profile.visibility.showTags ? null : (
            <GiveTagBox
              onGive={async (label) => {
      ...
      {owner ? null : (
        <div className="border-t border-gray-500 p-3">
          ...
          <ProfileCommentBox
            ...
            closedNotice={
              visibility.showProfileComments ? null : 'COMMENTS ON THIS PROFILE ARE SWITCHED OFF BY THE OWNER.'
            }
            onSubmit={onComment}
          />
        </div>
...
/** The "give this profile a tag" box: tags arrive hidden, the owner reveals them. */
function GiveTagBox({ onGive }: { onGive: (label: string) => Promise<void> }) {
  ...
  async function handleGive() {
    ...
    try {
      ...
      setLabel('');
      setStatus('TAG GIVEN - THE OWNER DECIDES WHETHER IT SHOWS.');
    } catch (caught) {
