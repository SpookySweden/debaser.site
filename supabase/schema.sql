...
-- document, and while one is open its editors merge their changes over a Realtime broadcast channel
-- and a row is written only when somebody saves (app/lib/lore/supabase-lore-repository.ts). So the
-- table holds two copies of the same writing, on purpose:
--
...
--
-- Neither is the other's source of truth: one save writes both, out of the same document. The state
-- is text rather than bytes because it is written and read by one client at a time - the merging
-- happens over the channel, never in the column - and text keeps the row legible in the dashboard.
--
-- Who may do what: anybody may read a page, and only a signed-in account may open or file one. That
-- is the opposite of the board, which takes posts from guests, and it is deliberate - a keystroke in
-- a shared document has nowhere to go without a name on it.
--
-- Nothing here is added to `supabase_realtime`, and nothing here needs to be: the editors talk over
-- a broadcast channel rather than over the table's change feed, and a publication entry is what a
-- `postgres_changes` subscription needs. This feature therefore cannot be the table that takes a
-- channel quiet, which is the failure mode section 9 warns about.

create table if not exists public.lore_pages (
  id uuid primary key default gen_random_uuid(),
  -- The page's address, and the key a link names: lower-case words joined by single hyphens.
  slug text not null unique,
  title text not null,
  summary text not null default '',
  -- The writing, twice: the document an editor opens, and the plain text a reader is shown.
...
-- The database's copy of `isLoreSlug` in app/lib/lore/pages.ts: the site refuses a title that makes
-- no address before it ever asks the table, and the table refuses one that got past it.
...
-- been answered. It is the whole of the arcade's storage. A match in progress writes nothing, because
-- a match is two browsers on one realtime broadcast channel named after the invitation (see
-- app/lib/games/channel.ts) - nothing about a game is kept anywhere, and nothing about it can go
-- stale.
--
-- Readable only by the two accounts a row names, and writable by either of them: the sender inserts
-- it, the recipient answers it (`status`), and both may remove it - the sender cancelling, the
-- recipient clearing one they have answered. That is what keeps a match private: the room's name is
-- the row's id, and only the pair can read the id.
--
...
-- `/games?invite=<id>` - which answers the invitation and starts the game. Read by
-- app/lib/notifications/supabase-notifications-repository.ts and app/lib/games/supabase-games-repository.ts.
...
-- Realtime: an accepted invitation has to reach the sender's screen without a reload.
do $$
begin
  if not exists (
    select 1 from pg_publication_tables
...
--   select to_regclass('public.game_invites');
--   select column_name from information_schema.columns
--     where table_schema = 'public' and table_name = 'forum_notifications' and column_name in ('game_id','invite_id');
--   select policyname, cmd from pg_policies where tablename = 'game_invites' order by policyname;
--   select table_name from pg_publication_tables
